...
class HomeActivity : BaseActivity() {
    ...
    fun navigate(view: View) {
        when (view.id) {
            ...
            R.id.liveStream, R.id.startLiveStream  -> {

                val view = layoutInflater.inflate(R.layout.dialog_livestream, null);

                AlertDialog.Builder(this).setMessage(R.string.live_stream)
                    .setView(view)
                    .setPositiveButton(
                        R.string.start
                    ) { dialog, id ->
                        SectionActivity.newInstance(
                            this,
                            Section.LIVE_STREAM,
                            view.code.text.toString()
                        )
                    }
